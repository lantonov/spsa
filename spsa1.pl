...
sub run_spsa
{
    ...
    # STEP. Prepare the initial thetas for reference engine 2
...
#                 $var_R{$name}      = $var_a{$name} / $var_c{$name} ** 2;
                 ...
                 $var_eng1{$name} = min(max($var_value{$name}, $var_min{$name}), $var_max{$name});

				 print "Iteration: $iter, variable: $name, value: $var_value{$name}\n";
             }
        }

        # STEP. Play two games (with alternating colors) and obtain the result (2, 1, 0, -1, -2) from eng1 perspective.
        my $result = ($simulate ? simulate_2games(\%var_eng1, \%var_eng2) : engine_2games(\%var_eng1));
		$result = 0.5 if $result == 0;
        $result_inc = $result_inc + $result;
        my $cost    = 1 - 1 / (1 + 10 ** (-$result_inc / 400));
        $coeff = 5.0 * ($cost - 0.1) * $cost; 
        # STEP. Apply the result
        {
            lock($shared_lock);

            my $logLine = "$iter,$cost,$coeff\n";

            foreach $row (@variables)
            {
                my $name = $row->[$VAR_NAME];
                
                $shared_theta{$name} += $coeff * $result / $var_delta{$name};
